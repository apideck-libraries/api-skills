...
- **Implementation difficulty:** moderate — API Key Authentication — Credential Generated by the Consumer's DualEntry Administrator
- **Vendor partnership required:** no — Apideck holds an approved partnership with DualEntry, so no application or certification is required from you.
- **Apideck-managed credentials:** not available — Each consumer supplies their own API key; there is no shared Apideck credential.
- **Account type required:** Any DualEntry plan. Multi-company tenants supported; scope to a Default Company or the whole tenant.
- **Consumer access level:** Organisation administrator — only an admin can generate the API key, via Settings > Organization > Developer access.
- **Sandbox:** available ([signup](https://docs.dualentry.com/developers/guides/how-to-authenticate-your-requests)) — A separate Development environment at api-dev.dualentry.com with its own organisations and API keys; a production key will not authenticate there.
- **Costs:** No separate API fee — bundled into the DualEntry subscription, which lists Open API as a plan feature.
- **Rate limits:** 300 requests/min per API key across all endpoints; 60 requests/min per endpoint; invoice create/update capped at 2/min.
- **Authentication:** Sent in the X-API-KEY header, not OAuth. The key is a long-lived organisation credential with no expiry.
- **Webhooks:** No webhooks — not supported by DualEntry's API.
...
> Facts synced from Apideck's connector metadata API — `GET /connector/connectors/dualentry` (`overview` field) is the live, authoritative version.
