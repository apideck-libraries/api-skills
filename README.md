...
The sync runs automatically from Apideck's internal side: whenever a connector overview is updated upstream, an internal workflow re-runs the sync against the changed sources, regenerates the skills, validates, and opens a PR here with the refreshed content. Manual `node connectors/sync-overviews.js` runs are only needed for local development.
